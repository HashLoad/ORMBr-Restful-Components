        ...
        begin
          ...
          begin
            ...
            if oORMBr.Installations[iVersion].CompilePackage(sDirPackage + NomePacote, sDirLibrary, sDirLibrary) then
              Logar(Format('Pacote "%s" compilado.', [framePacotes1.Pacotes[iDpk].Caption]))
            else
            begin
              Inc(FCountErros);
              Logar(Format('Erro ao compilar o pacote "%s".', [framePacotes1.Pacotes[iDpk].Caption]));
...
begin
  ...
  begin
    ...
    Application.MessageBox(
      'Para continuar escolha a versão do Delphi para a qual deseja instalar o ORMBr REST Componentes.',
      'Erro.',
      MB_OK + MB_ICONERROR
    );
  end;
